...
`mehboob.dev`, registered at Cloudflare Registrar and on Cloudflare nameservers.

`.dev` is on the HSTS preload list, so browsers refuse plain HTTP for it before a request
is ever made. Nothing to configure; just don't expect an http:// link to work.

The domain is written in exactly one place, `site:` in `astro.config.mjs`. Canonicals,
`og:url`, `og:image`, the JSON-LD blocks, the RSS item links and `llms.txt` all derive
from it. The one copy that does not is `public/og.png`, which has the domain rendered
into the artwork as pixels, so that has to be regenerated by hand if the domain changes.

(An earlier draft of this file pointed at `mehboobali.dev`. That was aspirational and
was never checked; someone else registered it in May 2026.)
